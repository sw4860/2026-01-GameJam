...
public class ItemAcquiredPopup : MonoBehaviour
{
    ...
    public void Show(string acquiredText, string description)
    {
        EnsureUi();

        if (titleText != null)
        {
            titleText.text = string.IsNullOrWhiteSpace(acquiredText) ? "[아이템을 획득했다.]" : acquiredText;
        }
    ...
    private void EnsureUi()
    {
        ...
        panelRect.anchorMin = new Vector2(0.24f, 0.64f);
        panelRect.anchorMax = new Vector2(0.76f, 0.90f);
        panelRect.offsetMin = Vector2.zero;
        panelRect.offsetMax = Vector2.zero;

        titleText = CreateText("Title", panelObject.transform, 22, FontStyle.Bold, titleColor, new Vector2(0f, 0.56f), Vector2.one);
        titleText.rectTransform.offsetMin = new Vector2(28f, 2f);
        titleText.rectTransform.offsetMax = new Vector2(-28f, -8f);

        bodyText = CreateText("Description", panelObject.transform, 18, FontStyle.Normal, bodyColor, Vector2.zero, new Vector2(1f, 0.60f));
        bodyText.rectTransform.offsetMin = new Vector2(28f, 14f);
        bodyText.rectTransform.offsetMax = new Vector2(-28f, -4f);
    }
