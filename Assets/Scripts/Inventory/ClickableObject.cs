...
public class ClickableObject : MonoBehaviour, IPointerDownHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    {
        Inventory.PickupItem selectedItem = Inventory.Instance?.GetSelectedItem();
        if (selectedItem != null)
        {
            if (TryUseSelectedItem(selectedItem)) return;
        }

        Examine();
    }

    private bool TryUseSelectedItem(Inventory.PickupItem item)
    {
        if (string.IsNullOrWhiteSpace(requiredItemId)) return false;

        bool correctItem = CanUseItem(item.Id);
        bool handled = TryUseItem(item.Id);
        
        if (!handled) return false;

        if (correctItem)
        {
            // Trigger specific visual changes if necessary
            if (objectId == "safe")
            {
                if (Inventory.Instance != null)
                {
                    Inventory.Instance.safeOpen = true;
                    Inventory.Instance.OpenSafeVisual();
                }
            }
            else if (objectId == "cabinet") // Ensure this ID matches your setup
            {
                if (Inventory.Instance != null)
                {
                    Inventory.Instance.cabinetOpen = true;
                    Inventory.Instance.OpenCabinetVisual();
                }
            }

            if (consumeRequiredItem)
            {
                Inventory.Instance?.RemoveCollectedItem(item);
            }
        }
        return true;
    }
    ...
    public bool Examine()
    {
        ...
        {
            StaticValues.SetFlag(stateFlagOnExamine);
        }

        bool showingAfterState = !string.IsNullOrWhiteSpace(stateFlagOnUse) &&
                                 StaticValues.HasFlag(stateFlagOnUse) &&
    ...
    public bool TryUseItem(string itemId)
    {
        ...
        {
            StaticValues.SetFlag(stateFlagOnUse);
        }
    ...
    {
        if (!string.IsNullOrWhiteSpace(speakerName)) return speakerName;
        return displayName;
    }
    ...
    private string GetExamineMessage()
    {
        ...
        if (!string.IsNullOrWhiteSpace(examineText)) return examineText;
